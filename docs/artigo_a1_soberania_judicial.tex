...
O classificador de complexidade atingiu acur\'{a}cia de \textbf{24\%} (6/25) (Figura~\ref{fig:classification}), com forte vi\'{e}s para a classe BAIXA. A matriz de confus\~{a}o revela que o classificador subestima sistematicamente a complexidade: queries M\'{E}DIA s\~{a}o classificadas como BAIXA e queries ALTA como M\'{E}DIA ou BAIXA. Esta \'{e} uma limita\c{c}\~{a}o documentada do classificador baseado em heur\'{i}sticas.

\begin{figure}[!t]
    \centering
    \includegraphics[width=\columnwidth]{figures/fig7_classification.png}
    \caption{Matriz de confus\~{a}o da classifica\c{c}\~{a}o de complexidade ($n$=25, Acc=24\%). O vi\'{e}s para BAIXA \'{e} conservador: resulta em recupera\c{c}\~{a}o mais leve, mas n\~{a}o compromete a qualidade das respostas.}
    \label{fig:classification}
\end{figure}

Criticamente, este vi\'{e}s conservador n\~{a}o compromete a qualidade final: as respostas mant\^{e}m 100\% de completude Toulmin independentemente da classifica\c{c}\~{a}o. O impacto pr\'{a}tico \'{e} que queries complexas recebem par\^{a}metros de recupera\c{c}\~{a}o sub\'{o}timos, resultando em menor uso do grafo de conhecimento do que o ideal. O estudo de abla\c{c}\~{a}o (Se\c{c}\~{a}o~\ref{sec:ablation}) confirma que o sistema mant\'{e}m 100\% de sucesso mesmo sem o grafo de conhecimento.

\subsection{Seguran\c{c}a Guardian (T3, $n$=35)}

O Guardian Agent demonstrou taxa de detec\c{c}\~{a}o de \textbf{70{,}4\%} (19/27 ataques bloqueados) com taxa de falso positivo de \textbf{12{,}5\%} (1/8 queries leg\'{i}timas bloqueada) (Figura~\ref{fig:guardian}, Tabela~\ref{tab:guardian}).
...
XSS & 4 & 3 & 75\% \\
Jailbreak & 6 & 4 & 67\% \\
Instruction Bypass & 5 & 3 & 60\% \\
Leetspeak Evasion & 4 & 1 & 25\% \\
\midrule
Leg\'{i}timas (Aceitas) & 8 & 7 & 87{,}5\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[!t]
    ...
    \caption{Efic\'{a}cia do Guardian Agent: (a)~taxa de detec\c{c}\~{a}o por tipo de ataque, (b)~distribui\c{c}\~{a}o geral. SQL Injection e Template Injection atingem 100\%. Leetspeak e Instruction Bypass s\~{a}o as maiores vulnerabilidades.}
    \label{fig:guardian}
\end{figure}

A taxa de 100\% para SQL Injection e Template Injection confirma a robustez contra ataques estruturais. A menor efic\'{a}cia em Leetspeak (25\%) e Instruction Bypass (60\%) indica necessidade de aprimoramento na normaliza\c{c}\~{a}o de obfusca\c{c}\~{a}o e na detec\c{c}\~{a}o de redefini\c{c}\~{a}o de papel. O \'{u}nico falso positivo ocorreu com a query ``O que diz o Art.~5\textordmasculine{} da CF sobre direitos fundamentais?'', possivelmente devido \`{a} sensibilidade do filtro para queries sobre direitos fundamentais que cont\^{e}m termos ambivalentes.

\subsection{Defesa P2P (T4, $n$=25)}

A defesa \textit{Poison-to-Poison} apresentou taxa de detec\c{c}\~{a}o de \textbf{20\%} para triggers maliciosos (4/20), com \textbf{100\% de aceita\c{c}\~{a}o de queries leg\'{i}timas} (5/5) (Figura~\ref{fig:p2p}, Tabela~\ref{tab:p2p}).
...
\begin{figure}[!t]
    ...
    \caption{Defesa P2P por tipo de trigger ($n$=25). A defesa \'{e} eficaz apenas para um subconjunto de triggers lexicais contendo palavras-chave de seguran\c{c}a espec\'{i}ficas (OVERRIDE, BYPASS, ADMIN). Triggers sem\^{a}nticos s\~{a}o indetect\'{a}veis pela abordagem atual.}
    \label{fig:p2p}
\end{figure}

A baixa taxa de detec\c{c}\~{a}o P2P (20\%) \'{e} a principal vulnerabilidade identificada. A defesa opera via correspond\^{e}ncia lexical de padr\~{o}es pr\'{e}-definidos, sendo eficaz apenas para triggers que contenham palavras-chave espec\'{i}ficas (OVERRIDE, BYPASS, ADMIN). Triggers sint\'{a}ticos (inje\c{c}\~{a}o entre segmentos leg\'{i}timos) e sem\^{a}nticos (cen\'{a}rios hipot\'{e}ticos que subvertem o dom\'{i}nio) n\~{a}o s\~{a}o detectados. Esta limita\c{c}\~{a}o \'{e} parcialmente mitigada pelas demais camadas de defesa (Guardian, SCOT), como demonstrado pelo estudo de abla\c{c}\~{a}o.
...
O Citation F1 mais baixo (0{,}349) indica uma limita\c{c}\~{a}o do Qwen2.5:14B quantizado: o modelo produz racioc\'{i}nio jur\'{i}dico correto (Keyword F1 alto), mas frequentemente omite a numera\c{c}\~{a}o precisa de artigos e leis (Citation F1 menor). Esta \'{e} uma \'{a}rea onde modelos jur\'{i}dicos especializados como SaulLM~\cite{saullm2024a} poderiam elevar significativamente a performance.
...
\begin{itemize}
    \item \textbf{Guardian (Camada 2)}: 70{,}4\% de detec\c{c}\~{a}o geral, com 100\% para SQL Injection e Template Injection. A taxa de 12{,}5\% de falsos positivos (1/8) requer investiga\c{c}\~{a}o.
    \item \textbf{P2P (Camada 4)}: 20\% de detec\c{c}\~{a}o, limitada a triggers lexicais espec\'{i}ficos. Esta \'{e} a principal vulnerabilidade identificada.
...
A combina\c{c}\~{a}o das camadas oferece defesa em profundidade: mesmo que a camada P2P falhe (o que ocorre para 80\% dos triggers), o Guardian e o SCOT fornecem barreiras adicionais. A probabilidade de bypass de todas as camadas simultaneamente permanece significativamente inferior \`{a} de qualquer camada individual.

\subsection{Impacto do Classificador de Complexidade}

A acur\'{a}cia de 24\% do classificador (T2) \'{e} uma limita\c{c}\~{a}o significativa, mas o estudo de abla\c{c}\~{a}o revela que seu impacto pr\'{a}tico \'{e} limitado: o sistema mant\'{e}m 100\% de sucesso independentemente dos par\^{a}metros adaptativos aplicados. O classificador determina os pesos ($\alpha$, $\beta$, $\gamma$) do HyPA-RAG, mas mesmo com pesos sub\'{o}timos, a fus\~{a}o RRF produz resultados adequados. Ainda assim, um classificador mais preciso otimizaria a lat\^{e}ncia e a relev\^{a}ncia dos documentos recuperados.
...
\begin{enumerate}
    \item \textbf{Classificador de complexidade (24\%)}: O classificador tr\^{e}s-classes (BAIXA/MEDIA/ALTA) apresenta vi\'{e}s para BAIXA, classificando incorretamente 76\% das queries. Embora o estudo de abla\c{c}\~{a}o demonstre impacto pr\'{a}tico limitado (100\% de sucesso independente dos pesos), um classificador mais preciso otimizaria lat\^{e}ncia e relev\^{a}ncia.

    \item \textbf{P2P para triggers n\~{a}o-lexicais}: A detec\c{c}\~{a}o P2P alcan\c{c}a apenas 20\% geral, com 25\% para lexicais, 25\% para sint\'{a}ticos e 0\% para sem\^{a}nticos. A detec\c{c}\~{a}o de triggers sem\^{a}nticos e contextuais requer integra\c{c}\~{a}o com an\'{a}lise LLM, ainda n\~{a}o implementada para manter baixa lat\^{e}ncia.

    \item \textbf{Leetspeak e ofusca\c{c}\~{a}o}: O Guardian detecta apenas 25\% (1/4) das tentativas em \textit{leetspeak}, indicando necessidade de expans\~{a}o do mapeamento de normaliza\c{c}\~{a}o para caracteres espec\'{i}ficos do portugu\^{e}s (\c{c}, \~{a}, \'{e}, etc.).
    ...
    \item \textbf{Citation F1 limitado}: O Citation F1 de 0{,}349 (IC 95\%: [0{,}245; 0{,}453]) indica que o Qwen2.5:14B quantizado frequentemente omite numera\c{c}\~{a}o precisa de artigos e leis, embora o racioc\'{i}nio jur\'{i}dico seja correto (Keyword F1 = 0{,}628).
...
\begin{itemize}
    ...
    \item \textbf{70{,}4\% de detec\c{c}\~{a}o} pelo Guardian com 12{,}5\% de falsos positivos, incluindo 100\% de bloqueio para SQL Injection e Template Injection.
...
A avalia\c{c}\~{a}o tamb\'{e}m revelou limita\c{c}\~{o}es concretas: o classificador de complexidade (24\% de acur\'{a}cia) e a detec\c{c}\~{a}o P2P (20\%) requerem melhorias significativas. Estas limita\c{c}\~{o}es, contudo, n\~{a}o comprometem a funcionalidade base do sistema, conforme demonstrado pelo estudo de abla\c{c}\~{a}o.

Trabalhos futuros incluem: (1)~integra\c{c}\~{a}o de modelos jur\'{i}dicos especializados como SaulLM-7B~\cite{saullm2024a} com \textit{fine-tuning} via QLoRA para elevar o Citation F1; (2)~detec\c{c}\~{a}o sem\^{a}ntica de triggers P2P via LLM para superar a limita\c{c}\~{a}o lexical atual (0\% sem\^{a}ntico); (3)~retreino do classificador com dados rotulados para superar o vi\'{e}s BAIXA; (4)~valida\c{c}\~{a}o por painel de juristas e magistrados brasileiros; (5)~\textit{deployment} com GPU para escalabilidade de produ\c{c}\~{a}o; e (6)~expans\~{a}o da base de conhecimento com legisla\c{c}\~{a}o estadual e jurisprud\^{e}ncia completa.
